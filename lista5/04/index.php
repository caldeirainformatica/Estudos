<html>
	...
	function gerarInput($input){
			if (!empty($_GET)){
				echo '<div class="container" style="margin-top: 60px">';
					echo '<form action="index.php">';
						echo '<label for="input">Você escolheu ' .$_GET['input']. ' entradas</label><br/>';
			...
			for ($cont = 1; $cont <= $_GET['input']; $cont++){
						echo '<label for="valor_' . $cont . '">Digite o valor da mercadoria</label><br/>';
						echo '<input type="number" id="valor_' . $cont . '" name="valor_'. $cont .'" required="required"><br/>';
	...
	function calcula(){
			...
			$media = 0;
			for ($cont = 1; $cont <= $_GET['input'] ; $cont++){
				$temp = $_GET['valor_'.$cont];
				$num = $num+$temp;
			}
				$media = $num/($cont-1);
				echo '<h1>O valor total das mercadorias é: ' . $num . '. A média das mercadorias é: ' . $media . '</h1>';
		}
